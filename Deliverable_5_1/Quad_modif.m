...
%       x = [x (t/ref_time)]';
      if (t == 0)
        x = [x 0]';  
      elseif (t < 20)
        x = [x (t/ref_time)/(t/ref_time)]'; %2*sin(t/ref_time*2*pi)]';
      else
        x = [x 3*(t/ref_time)/(t/ref_time)]'; 
      end
    ...
    function [xs, us] = trim(quad)
      ...
      LB = [-inf*ones(12,1); quad.thrustLimits(1,:)'];
      UB = [ inf*ones(12,1); quad.thrustLimits(2,:)'];
  ...
  methods (Static)
    function pos = pos(x)
      pos = x(Quad_modif.ind.pos, :);
    end
    function vel = vel(x)
      vel = x(Quad_modif.ind.vel, :);
    end
    function theta = theta(x)
      theta = x(Quad_modif.ind.theta, :);
    end
    function omega = omega(x)
      omega = x(Quad_modif.ind.omega, :);
    end
  end
