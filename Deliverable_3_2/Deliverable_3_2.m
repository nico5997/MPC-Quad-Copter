...
while (tmp < 2)
    ...
    for i = 1:N-1
        ...
        ux(:,i) = mpc_x.get_u(x(:,i), -2);
        uy(:,i) = mpc_y.get_u(y(:,i), -2);
        uz(:,i) = mpc_z.get_u(z(:,i), -2);
        uyaw(:,i) = mpc_yaw.get_u(yaw(:,i), pi/4);

        if (tmp == 0)
           ux_tmp(:,i) = mpc_x.get_u(x(:,i), -2);
           uy_tmp(:,i) = mpc_y.get_u(y(:,i), -2);
           uz_tmp(:,i) = mpc_z.get_u(z(:,i), -2);
           uyaw_tmp(:,i) = mpc_yaw.get_u(yaw(:,i), pi/4); 
        end
...
S_x = stepinfo(x(4,:),Ts*(1:N),-2,'SettlingTimeThreshold',0.02);
disp('Settling time of x is:')
S_x.SettlingTime
S_y = stepinfo(y(4,:),Ts*(1:N),-2,'SettlingTimeThreshold',0.02);
disp('Settling time of y is:')
S_y.SettlingTime
S_z = stepinfo(z(2,:),Ts*(1:N),-2,'SettlingTimeThreshold',0.02);
disp('Settling time of z is:')
S_z.SettlingTime
S_yaw = stepinfo(yaw(2,:),Ts*(1:N),pi/4,'SettlingTimeThreshold',0.02);
